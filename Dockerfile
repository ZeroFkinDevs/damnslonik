...
# Клонируем и собираем ngx_brotli модуль
RUN cd /tmp && \
...
# Собираем nginx с поддержкой динамических модулей и ngx_brotli
RUN cd /tmp && \
...
RUN apk del git gcc g++ make cmake pkgconfig zlib-dev
